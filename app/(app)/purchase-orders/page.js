...
export default async function PurchaseOrdersPage() {
  ...
  // Exclude POs belonging to archived projects
  const pos = (allPOs || []).filter(p => !p.projects?.archived)
  const archivedCount = (allPOs || []).length - pos.length
  ...
  return (
    <div className="grid">
      <div className="page-header">
        <div className="split">
          <div><h1>Purchase Orders</h1><p className="muted">{pos.length} purchase orders</p></div>
          <div className="toolbar">
            <Link href="/purchase-orders/archived"><button className="small">Archived{archivedCount ? ` (${archivedCount})` : ''}</button></Link>
          </div>
        </div>
      </div>
