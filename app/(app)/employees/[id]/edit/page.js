...
export default async function EditEmployeePage({ params }) {
  ...
  const [{ data: worker }, { data: rates }, { data: tickets }, { data: orientations }] = await Promise.all([
  ...
  return <EmployeeForm worker={worker} rates={rates || []} initialTickets={tickets || []} initialOrientations={orientations || []} />
}
