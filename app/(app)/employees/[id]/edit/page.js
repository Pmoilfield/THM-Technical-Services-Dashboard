...
export default async function EditEmployeePage({ params }) {
  ...
  const [{ data: worker }, { data: rates }, { data: tickets }, { data: orientations }, { data: holidays }, { data: certifications }] = await Promise.all([
    ...
    supabase.from('worker_certifications').select('*').eq('worker_id', id).order('cert_type'),
  ])

  if (!worker) notFound()

  return <EmployeeForm worker={worker} rates={rates || []} initialTickets={tickets || []} initialOrientations={orientations || []} initialHolidays={holidays || []} initialCertifications={certifications || []} />
}
